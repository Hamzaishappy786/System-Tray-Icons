...
The binary is produced at `target\release\tray_guardian.exe`. The bubble icon (`assets/handyman.jpg`) is embedded into the binary at compile time — no external files needed at runtime. To use a different icon, replace `assets/handyman.jpg` and rebuild.
